...
class FunctionStatic {
	...
	public static int[] funcStatic(int number) {
		int answer[];
		int k = 0;
...
public class Odd_class_Static {
	public static void main(String[] args) {
		...
		while ((number = sc.nextInt()) != 0) {
			...
			// answer 배열을 출력
			for (int k = 0; k < FunctionStatic.funcStatic(number).length; k++) { // 범위 주의
				System.out.print(FunctionStatic.funcStatic(number)[k] + " ");
			}
			System.out.println("");
			System.out.println("");
			System.out.print("[정수 n을 입력하세요](0을 누르면 종료)");
